...
上边距的调节方式与宽度权重类似，写法为 `t=<上边距>`，上边距以 `em` 为单位，表示除去页面边距后的页面宽度的 $\frac{1}{100}$。
...
<Cr.RenderPropsReference filters={['time_lining']} />
...
### 按音符数微调小节宽度 {#note-count-lining}

<C.MinVersion min='1.15.8' />

<Cr.RenderPropsReference filters={['note_count_lining']} noMinVersion />

有的人认为，所有小节宽度都一致的乐谱容易形成很强的「网格感」，看起来「了无生气」。为解决这个问题，Sparks NMN 提供了一个自动根据音符数微调小节宽度的功能，大多数情况下可以使行与行之间的小节线略有点不对齐，打破这种「网格感」。例如《数鸭子》的前两行，
...
此功能可以和「基于时值自动设置小节宽度」同时使用，它们的效果会叠加。

如果两行的节奏型完全一样，各小节音符数毫无差别，那么它们仍然会对齐。
...
<Cr.RenderPropsReference filters={['legacy_positioning']} />
...
接下来，我们将带着例子介绍一些零碎的样式选项，它们是非常容易理解的。
