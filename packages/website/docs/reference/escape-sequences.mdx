...
- 在内容类型为“文本”或“空格分割”的指令行中（例如 `P` `Rp` `Dt` `T` 等）。<C.MinVersion min='Sparks NMN 1.14.16' inline />
- 在任何双引号引起的字符串中。
...
| `\/` | `/` | 正斜杠<br /><C.MinVersion min='Sparks NMN 1.14.16' inline /> |
| `\x3C` | `<` | `\x` 后跟两位十六进制，表示 ASCII 代码对应的字符。不区分大小写。<br />由于一些问题，目前无法正常工作。 |
| `\u951F` | `锟` | `\u` 后跟四位十六进制，表示 UTF-16 LE 代码对应的字符。不区分大小写。 |
| `\*` | | 空白，不输出任何内容<br /><C.MinVersion min='Sparks NMN 1.14.16' inline /> |
| `\\` | `\` | 反斜杠 |
