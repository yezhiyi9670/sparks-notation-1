...
|<C.MinVersion max='soon' inline>尾注</C.MinVersion><br /><C.MinVersion min='soon' inline>脚注</C.MinVersion>| |`DocFootnote`|`Df<[<编号>]?>: <文本>`，例如 `Df[1]: 这里的符号表示换气` |
...
&#xFEFF;<C.MinVersion max='soon' inline>目前，尾注文本 `Df` 会显示在整个乐谱的末尾，因此也可以添加在整个文档最后，而不必在开头。</C.MinVersion>

&#xFEFF;<C.MinVersion min='soon' inline>在开启[页面精排](/docs/tutorial-layout/pagination)后，脚注将会显示在**第一页**的底部。若要在文档底部添加文本注释，请直接在乐谱末尾加入一个[大段文本章节](/docs/tutorial-basics/multiarticle#text-article)。</C.MinVersion>
...
如果要在这些乐谱信息行中添加一些无法直接表示的字符，可以使用**转义序列**。转义序列是由 `\` 字符开始的序列，用来代表一些特殊的字符。如果你对计算机程序语言并不熟悉，因而不了解转义字符，[这里有一份简单的介绍](/docs/reference/escape-sequences#escape-list)。
