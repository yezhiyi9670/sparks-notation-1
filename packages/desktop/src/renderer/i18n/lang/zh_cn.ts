...
export default flattenI18nData({
	...
	"new_document": `Dt: 新文档
Ds: 副标题
Dv: Version ${window.Versions.app}
Da[作词]: ???
Da[作曲]: ???
P: 1=C 4/4
...
`,
	...
	"hint": {
		...
		"welcome": {
			...
			"line": {
				...
				"3": "要开始创建文档，直接在界面上的代码编辑框内输入即可。如果你不清楚如何编写代码，可以点击下面的链接查看指南。不要忘了保存哦！",
		...
		"printEssence": {
			"title": "如何打印",
			"line": {
				"1": `在点击“打印”按钮后，${appName} 将会导出临时的 HTML 预览文件，并在浏览器中打开。你可在设置中选择浏览器。`,
				"2": "文件打开后，你的浏览器应当会自动弹出打印预览对话框，你可以选择打印选项并执行打印。如果没有弹出，请尝试按 Ctrl + P 来手动触发。",
				"3": "HTML 预览文件的存放位置以及使用的浏览器取决于设置。如果打印排版不太对或者出现错乱，请考虑使用基于 Chromium 的浏览器进行打印。",
			},
			"confirm": "打印"
		},
		"rate": {
			...
			"line": {
				...
				"2": "如果你喜欢此应用，请考虑给予五星好评 / Github Star，或通过爱发电平台对开发者进行捐赠。你的支持是开发者不断改进的动力。"
			},
			"link": {
				"github": "Github",
				"donate": "捐赠"
	...
	"settings": {
		...
		"item": {
			...
			"allowUpdateCheck": {
				"title": "允许检查更新",
				"desc": "开启后，启动时将自动联网检查是否有新版本。目前，软件不会自动下载或安装更新。\n除了进行完全匿名的访问量计数外，服务器不会记录你的请求和你的设备/网络信息。",
				"choice": {
					"off": "关",
					"on": "开"
				}
			...
			"browser": {
				"title": "首选浏览器",
				"desc": "打印时使用的浏览器可执行文件。若留空，将使用系统默认浏览器进行打印。\n一般而言，基于 Chromium 的浏览器有较好的效果，其他浏览器可自行尝试。",
				// "desc": "打印及导出 PDF/PNG 时使用的浏览器可执行文件。\n一般而言，基于 Chromium 的浏览器有较好的效果，其他浏览器可自行尝试。\n若留空，打印时将使用系统默认浏览器，导出 PDF/PNG 功能将无法使用。",
			},
			"tempHtmlLocation": {
				"title": "临时 HTML 文件位置",
				"desc": "打印时创建临时 HTML 的位置。\n临时文件夹不会被定期清理，但是每次使用都会覆盖上一次的文件。",
	...
	"link": {
		...
		"github": 'https://github.com/yezhiyi9670/sparks-notation-1',
		"donate": 'https://notation.sparkslab.art/donate/',
		"tutorial": "https://notation.sparkslab.art/docs/intro"
	},
})
