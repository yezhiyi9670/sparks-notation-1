...
import { SparksNMNEditor } from './base/SparksNMNEditor'
import { StatusDisplayMode } from './status/display-mode'
import AceEditor from 'react-ace'
import { createDethrottledApplier } from '@sparks-notation/util/event'
...
import { I18n, LanguageArray } from '@sparks-notation/core/i18n'
import { basenameName } from '@sparks-notation/util/basename'
import $ from 'jquery'
...
import { MusicSection, NoteCharMusic } from '@sparks-notation/core/parser/sparse2des/types'
...
export const __IntegratedEditor = React.forwardRef<IntegratedEditorApi, __Props>((props: __Props, ref) => {
	...
	const handleNew = useMethod((defaultContent: string) => {
		setValue(defaultContent)
		updateResult(defaultContent)
	...
	useImperativeHandle(ref, () => ({
		...
		triggerNew: (defaultContent: string) => {
			return handleNew(defaultContent)
		},
		triggerOpen: (data) => {
...
export interface IntegratedEditorApi {
	...
	triggerNew: (defaultContent: string) => void
